...
namespace gazebo
{
  class ObstaclesMover : public ModelPlugin
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/)
    {
      ...
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(
          std::bind(&ObstaclesMover::OnUpdate, this));
      
      this->old_secs =ros::Time::now().toSec();
      
      // Create a topic name
      std::string obstacles_pos_topicName = "/cmd_vel";
      ...
      if (!ros::isInitialized())
      {
        int argc = 0;
        char **argv = NULL;
        ros::init(argc, argv, "obstacles_rosnode",
            ros::init_options::NoSigintHandler);
      }
         
      // Create our ROS node. This acts in a similar manner to
      // the Gazebo node
      this->rosNode.reset(new ros::NodeHandle("obstacles_rosnode"));
      
      // Obstacles Pose
      ros::SubscribeOptions so =
        ros::SubscribeOptions::create<geometry_msgs::Twist>(
            obstacles_pos_topicName,
            1,
            boost::bind(&ObstaclesMover::OnRosMsg_Pos, this, _1),
            ros::VoidPtr(), &this->rosQueue);
      this->rosSub = this->rosNode->subscribe(so);
      
      // Spin up the queue helper thread.
      this->rosQueueThread =
        std::thread(std::bind(&ObstaclesMover::QueueThread, this));
  ...
  GZ_REGISTER_MODEL_PLUGIN(ObstaclesMover)
}
